...
stages {
    ...
    steps {
        ...
        sh "aws ecs create-service --cluster ${cluster} --service-name Web-App --task-definition ${task_def_arn} --region ${AWS_DEFAULT_REGION} --desired-count 1 --launch-type "FARGATE" --network-configuration awsvpcConfiguration={subnets=["subnet-05c077414f7d610c1"],securityGroups=["sg-0c5349ae977939c27"],assignPublicIp=ENABLED},assignPublicIp=ENABLED}"
    }
}
    }
}
